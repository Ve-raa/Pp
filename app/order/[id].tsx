      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          {order.items?.map((item) => (
            <View key={item.id} style={styles.orderItem}>
              <Text style={styles.itemPrice}>{item.price}د.إ × {item.quantity}</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            <Text style={styles.val}>{order.subtotal?.toFixed(2)}د.إ</Text>
            <Text style={styles.lbl}>المجموع الفرعي</Text>
          </View>
          {order.discount && order.discount > 0 ? (
            <View style={styles.row}>
              <Text style={[styles.val, { color: Colors.success }]}>-{order.discount?.toFixed(2)}د.إ</Text>
              <Text style={styles.lbl}>الخصم</Text>
            </View>
          ) : null}
          <View style={[styles.row, styles.totalRow]}>
            <Text style={styles.totalVal}>{order.total?.toFixed(2)}د.إ</Text>
